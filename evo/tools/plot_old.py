...
def traj(ax, plot_mode, traj, style='-', color='black', label="", alpha=1.0,start=0, end=-1):
    """
    ...
    x = traj.positions_xyz[start:end, x_idx]
    y = traj.positions_xyz[start:end, y_idx]
    

    if plot_mode == PlotMode.xyz:
        z = traj.positions_xyz[start:end, z_idx]
...
def traj_colormap(ax, traj, array, plot_mode, min_map, max_map, title="",start=0, end=-1):
    """
    ...
    pos = traj.positions_xyz[start:end,:]
...
def traj_xyz(axarr, traj, style='-', color='black', label="", alpha=1.0,
             start_timestamp=None,start=0,end=-1):
    """
    plot a path/trajectory based on xyz coordinates into an axis
    ...
    ylabels = ["$x$ (m)", "$y$ (m)", "$z$ (m)"]
    for i in range(0, 3):
        axarr[i].plot(x[start:end], traj.positions_xyz[start:end, i], style, color=color,
...
def traj_rpy(axarr, traj, style='-', color='black', label="", alpha=1.0,
    ...
    for i in range(0, 3):
        axarr[i].plot(x[start:end], np.rad2deg(traj.orientations_euler[start:end, i]), style,
