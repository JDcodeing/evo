    ...
    if args.plot or args.save_plot or args.serialize_plot:
        ...
        if args.ref:
            ...
            plot.traj_xyz(
                ...
                alpha=SETTINGS.plot_reference_alpha,start=pltstart,
                      end=pltend)
            plot.traj_rpy(
        ...
        for name, traj in trajectories.items():
            ...
            if SETTINGS.plot_usetex:
                short_traj_name = short_traj_name.replace("_", "\\_")
            plot.traj(ax_traj, plot_mode, traj, '-', color, short_traj_name,start=pltstart,end=pltend)
            ...
            plot.traj_xyz(axarr_xyz, traj, '-', color, short_traj_name,
                          start_timestamp=start_time,start=pltstart,end=pltend)
            ...
            plot.traj_colormap(ax, traj, speeds, plot_mode, min_map=min(speeds),
                           max_map=max(max(speeds),10), title="speed mapped onto trajectory",start=pltstart,end=pltend)
